...
@section('content')
<style>
    .st{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:16px;margin-bottom:20px}
    .st-card{background:var(--bg-card);border:1px solid var(--border-color);border-radius:10px;padding:18px}
    .st-head{font-size:10px;color:var(--text-secondary);text-transform:uppercase;letter-spacing:.8px;font-weight:600;margin-bottom:8px}
...
<div class="st">
    <div class="st-card">
        <div class="st-head">Média Geral ({{ date('Y') }})</div>
        <div class="st-val" style="color:{{ $notas->avg('mg') >= 10 ? 'var(--accent-green)' : '#FB923C' }}">
            {{ $notas->count() > 0 ? rtrim(rtrim(number_format($notas->avg('mg'), 2, ',', ' '), '0'), ',') : '—' }}
        </div>
    </div>
    <div class="st-card">
        <div class="st-head">Disciplinas</div>
        <div class="st-val">{{ $notas->count() }}</div>
    </div>
    <div class="st-card">
        <div class="st-head">Aprovadas (mg ≥ 10)</div>
        <div class="st-val" style="color:var(--accent-green)">{{ $notas->where('mg', '>=', 10)->count() }}</div>
    </div>
    <div class="st-card">
        <div class="st-head">Por melhorar (mg < 10)</div>
        <div class="st-val" style="color:#FB923C">{{ $notas->where('mg', '<', 10)->count() }}</div>
    </div>
</div>
