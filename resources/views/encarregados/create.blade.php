...
@section('title', 'Novo Encarregado')
@section('page-title', 'Novo Encarregado de Educação')
...
<form action="{{ route('admin.encarregados.store') }}" method="POST">
    @csrf
    <div class="form-card">
        ...
        <div class="form-grid">
            <div class="form-group full-width">
                ...
                <input type="text" name="nome" value="{{ old('nome') }}" class="form-input @error('nome') is-invalid @enderror" placeholder="Ex: Joana Mendes" required>
            ...
            <div class="form-group">
                ...
                <input type="text" name="telefone" value="{{ old('telefone') }}" class="form-input @error('telefone') is-invalid @enderror" placeholder="Ex: +245 955 000 000">
                @error('telefone')<div class="form-error">{{ $message }}</div>@enderror
            </div>

            <div class="form-group">
                <label class="form-label">Email</label>
                <input type="email" name="email" value="{{ old('email') }}" class="form-input @error('email') is-invalid @enderror" placeholder="exemplo@email.com">
                @error('email')<div class="form-error">{{ $message }}</div>@enderror
            </div>

            <div class="form-group full-width">
                <label class="form-label">Endereço</label>
                <input type="text" name="endereco" value="{{ old('endereco') }}" class="form-input @error('endereco') is-invalid @enderror" placeholder="Ex: Bairro de Quelele, Bissau">
        ...
        <div class="form-grid">
            <div class="form-group full-width">
                <label class="checkbox-group">
                    ...
                    <span>Criar conta de acesso para o encarregado (login para acompanhar o filho)</span>
                </label>
                <div class="form-hint">Com a conta, o encarregado poderá ver as notas, faltas e avisos do filho.</div>
            </div>
        </div>
        ...
        <div class="form-actions">
            ...
            <a href="{{ route('admin.encarregados.index') }}" class="btn"><i class="fas fa-times"></i> Cancelar</a>
        </div>
    </div>
</form>
@endsection