...
@php
    $prof = auth()->user();
    $turmasDoc = $horarios->pluck('turma')->filter()->unique('id');
    $anoLetivo = $prof->ano_lectivo ?? $turmasDoc->pluck('ano_lectivo')->filter()->unique()->implode(', ');
    $turno = $turmasDoc->pluck('periodo')->filter()->unique()->implode(', ');
    $turmasNomes = $turmasDoc->pluck('nome_turma')->unique()->implode(', ');
    $horasSemana = $horarios->count() > 0 ? round($horarios->sum(fn ($a) => $a->hora_fim->diffInMinutes($a->hora_inicio)) / 60, 1) : 0;
@endphp
...
<div class="hz-doc">
    ...
    <div class="info-section">
        <div class="info-column">
            <p><strong>Professor:</strong> {{ $prof->name }}</p>
    ...
    <div class="schedule-card">
        <h2 class="schedule-title">HORÁRIO SEMANAL DO PROFESSOR</h2>
...
<div class="empty-card">
    ...
    <div>Não há aulas atribuídas ao seu horário.</div>
</div>
@endif
@endsection
