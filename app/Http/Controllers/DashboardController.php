...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        // Gestão
        if ($user->hasRole('admin') || $user->hasRole('diretor') || $user->hasRole('pctp') || $user->hasRole('financeiro')) {
            $data['pagamentosPendentes'] = Pagamento::where('status', 'pendente')->count();
        }

        if ($user->hasRole('admin') || $user->hasRole('diretor') || $user->hasRole('pctp')) {
        ...
        if ($user->isEncarregado()) {
            ...
            // Alertas de faltas (3+ faltas num mês)
